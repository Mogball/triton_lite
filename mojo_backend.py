...
def maybe_upcast_float32(convert_output: bool = True) -> Callable[[_T], _T]:
    ...
    def needs_upcast(var) -> bool:
        return (not config.triton.codegen_upcast_to_fp32
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        def wrapped(*args, **kwargs) -> str:
            ...
            result_dtype = (None if not any_needs_upcast else getattr(
                get_dtype_handler(), func.__name__)(*args, **kwargs))
            needs_downcast = result_dtype not in (torch.float32, None)
            downcast_string = (f".to[{triton_type(result_dtype)}]()"
...
class TritonOverrides(OpOverrides):
    ...
    def to_dtype(
        ...
        def _get_min_elements_per_thread(src_dtype: torch.dtype,
                                         dst_dtype: torch.dtype) -> int:
            ...
            assert not (
                src_dtype in fp8_dtypes and dst_dtype in fp8_dtypes
    ...
    @staticmethod
    def inline_asm_elementwise(*inputs,
                               asm,
                               constraints=None,
                               dtype=torch.float32,
                               is_pure=True,
    ...
    def index_expr(cls, expr, dtype):
        raise NotImplementedError(
            "ops.index_expr not implemented outside a kernel")

    @staticmethod
    def masked(mask, body, other):
        raise NotImplementedError(
            "ops.masked not implemented outside a kernel")
    ...
    def load_seed(name, offset):
        raise NotImplementedError(
            "ops.load_seed not implemented outside a kernel")
...
class TritonKernelOverrides(TritonOverrides):
    ...
    def _setup_libdevice_routing(cls):
        ...
        for fn_name in torch._inductor.utils.op_requires_libdevice_fp64:
            ...
            if fn_name == "sigmoid":
                ...
                fn = functools.partial(decomposition_router,
                                       _original_impl=original_impl,
                                       _fn_name=fn_name)
            ...
            fn = functools.partial(dtype_router,
                                   _original_impl=original_impl,
                                   _fn_name=fn_name)
            fn.__name__ = fn_name  # type: ignore[attr-defined]
            setattr(cls, fn_name, staticmethod(fn))
    ...
    def index_expr(cls, expr, dtype):
        ...
        dtype = dtype if dtype not in (torch.int32,
                                       torch.int64) else index_dtype
    ...
    def masked(mask, body, other):
        ...
        for node in nodes:
            for arg in node.args:
                if arg.target != "load" or should_unwrap_unspec_arg(
                        arg.args[1]):
                    need_where = True
                    break
...
class TritonKernel(SIMDKernel[TritonCSEVariable]):
    ...
    def init_cooperative_reduction(self):
        ...
        self.body.splice(
            ...
            """, )
        if any(not self._has_constant_mask(tree) for tree in self.range_trees
    ...
    def codegen_range_tree(self):
        ...
        if self.inside_reduction:
            if any(tree.is_loop for tree in self.range_trees):
                ...
                rn_bases = self._get_reduction_symbols("base",
                                                       integer=True,
                                                       nonnegative=True)
        ...
        *,
        ...
        for var in index_vars:
            ...
            if override_mask:
                ...
                prefix_matches = [
                    prefix_str[symt] for symt in TritonSymbols.block_types
                    if symbol_is_type(var, symt)
                ]
                assert len(prefix_matches) == 1, f"Ambiguous type: {var.name}"
                mask_vars.add(f"{prefix_matches[0]}mask")

        need_dense = (config.triton.dense_indexing or dense_indexing
                      or self._load_mask is not None) and index != 0
        ...
        if (block_ptr and self.allow_block_ptr and config.triton.use_block_ptr
                and not override_mask and not self._load_mask
                and len(mask_vars - dense_mask_vars) == 0
                and not self.is_indirect_indexing(index) and have_loop_vars
            ...
            def match_mod_div_block(
                ...
                if any(not sizevars.statically_known_multiple_of(
                        numel, max_block)
                ...
                block_offsets: list[sympy.Expr] = [
                    sympy_subs(expr, {
                        index_var:
                        TritonSymbols.get_block_offset(range_tree)
            ...
            def match_block_pointer() -> Optional[BlockPtrOptions]:
                index_relative_to_xyr_index = sympy_subs(
                    index, {
                        v: t.expr
                        for v, t in self.range_tree_nodes.items()
                    })
                ...
                range_symbols = OrderedSet(tree.symbol()
                                           for tree in range_trees)
                ...
                for tree, subexpr in zip(range_trees, index_subexprs):
                    ...
                    if len(range_symbols.intersection(
                            subexpr.free_symbols)) > 1:
                        return None
        ...
        if isinstance(index, sympy.Integer):
            ...
            return IndexingOptions(index_str, mask_vars, expand_str,
                                   has_rindex, index)
        ...
        return IndexingOptions(index_str, mask_vars, expand_str, has_rindex,
                               index)

    def codegen_block_ptr(self,
                          name: str,
                          var: str,
                          indexing: BlockPtrOptions,
                          other="") -> tuple[str, str]:
        ...
        if (self.inside_reduction and self.range_trees[-1].is_loop
            ...
            self.body.writeline(
                DeferredLine(
                    name,
                    f"{block_ptr} = {indexing.format(var, roffset=False)}"))
    ...
    def codegen_block_ptr_store_line(self,
        ...
        value = indexing.codegen_broadcast_and_reshape(value,
    ...
    def check_bounds(
        ...
        line = self.indirect_assert(index_str, "0" if lower else None,
                                    size_str, mask_str)
    ...
    def load(self, name: str, index: sympy.Expr):
        ...
        if self.is_broadcasted(original_index):
            ...
            def decide_later():
                if load_counts[name] > expected_count and (has_rindex or
                                                           indirect_indexing):
                    return "evict_last"
                return "evict_first"
            ...
            make_line = functools.partial(DelayReplaceLine, "<EP>",
                                          decide_later)
        else:
            ep = ""
        ...
        skip_l1_cache = (
            ...
            and not self.inside_reduction and not has_read_deps
        ...
        result_var = self.cse.generate(load_buffer,
                                       make_line(line),
                                       dtype=dtype)
        ...
        if append_broadcast:
            ...
            if indexing.mask_vars:
                if dtype.is_floating_point:
                    ...
                    zero = "0"
                other_val = (constant_repr(self._load_other)
                             if self._load_other else zero)
    ...
    def store(self,
        ...
        if isinstance(indexing, BlockPtrOptions):
            ...
            line = self.codegen_block_ptr_store_line(name, indexing, block_ptr,
                                                     value, other)
        elif mode is None:
        ...
        if not self.inside_reduction and self.cooperative_reduction:
            exit_stack.enter_context(
                self.guard_cooperative_store(name, self.stores))
    ...
    def guard_cooperative_store(self, name, buffer):
        ...
        buffer.writeline(
            DeferredLine(name, f"if rsplit_id == ({idx} % RSPLIT):"))
    ...
    def reduction_collapse_dims(self, buffer, value: str,
        ...
        return str(
            self.cse.generate(buffer,
                              triton_reshape(value, initial_shape,
                                             target_shape),
                              dtype=dtype))
    ...
    def reduction(
        ...
        def maybe_upcast(value: CSEVariable) -> CSEVariable:
            ...
            return (ops.to_dtype(value, torch.float32) if value.dtype in [
                torch.float16,
                torch.bfloat16,
            ] else value)
        ...
        result_var.mask_vars = OrderedSet(var for var in masks
                                          if not prefix_is_reduction(var[0]))
        cond = " & ".join(masks)
        ...
        if self.persistent_reduction:
            ...
            if reduction_type == "online_softmax_reduce":
                ...
                masked_value = [
                    _mask_value(v, d) for v, d in zip(value, default)
                ]
            else:
            ...
            if reduction_type in ("argmax", "argmin"):
                ...
                result_var = tuple(
                    ...
                    for value in self._welford(self.compute, mean, m2, weight,
                                               dim, dtype))
            elif reduction_type == "online_softmax_reduce":
                # All data is loaded to register anyway, no need to do
                # online softmax
                result_var = self.prepare_softmax_twopass_fallback(
                    dtype, value)
            else:
            ...
            accumulator = self.cse.namedvar(f"_{result_var}",
                                            dtype=torch_acc_type)
            default = ir.Reduction.default_accumulator(reduction_type,
                                                       src_dtype)
            default = self._map_tuple_or_scalar(constant_repr, default)
            ...
            if reduction_type in ("argmax", "argmin"):
                ...
                """)
                final_argreduce(self.post_loop_combine, result_var,
                                accumulator, accumulator_index)
            elif is_welford_reduction(reduction_type):
                result_var = self.welford_reduce(result_var, reduction_type,
                                                 value, where_cond, acc_type,
                                                 dtype)
            elif reduction_type == "online_softmax_reduce":
                ...
                if src_dtype == torch.bool:
                    ...
                    final_reduction_define(self.post_loop_combine,
                                           str(result_var), str(accumulator),
                                           None)
        ...
        if self.cooperative_reduction:
            ...
            if reduction_type in ("argmax", "argmin"):
                ...
                peer_idx = self.codegen_cooperative_reduction_peer_combine(
                    result_var, index_dtype,
                    torch.iinfo(index_dtype).max)
                final_argreduce(self.post_loop_store, result_var, peer_val,
                                peer_idx)
            elif is_welford_reduction(reduction_type):
                ...
                final_reduction_define(self.post_loop_store, str(result_var),
                                       peers, None)
            exit_stack.close()
    ...
    def _online_softmax_reduce(self, buffer, accumulator_max, accumulator_sum,
                               dim, dtype: torch.dtype):
        accumulator_max = self.reduction_collapse_dims(buffer, accumulator_max,
                                                       dtype)
        accumulator_sum = self.reduction_collapse_dims(buffer, accumulator_sum,
                                                       dtype)
        result_max, result_sum = [
            str(self.cse.newvar(dtype=dtype)) for _ in range(2)
        ]
        buffer.splice(f"""
    ...
    def _welford(self, buffer, mean, m2, weight, dim, dtype: torch.dtype):
        ...
        """
        mean, m2, weight = (self.reduction_collapse_dims(buffer, value, dtype)
        ...
        result_values = tuple(
            self.reduction_resize(value) for value in welford_results)
        return result_values

    def welford_reduce(self, result_var, reduction_type, value, where_cond,
                       acc_type, dtype):
    ...
    def online_softmax_reduce_final_reduction(self, buffer, result_max,
                                              result_sum, peer_max, peer_sum,
                                              dim, dtype):
        values = self._online_softmax_reduce(buffer, peer_max, peer_sum, dim,
                                             dtype)
    ...
    def codegen_cooperative_reduction_peer_combine(self, result_var, dtype,
        ...
        ws_name, ws_offset = self.cooperative_reduction_workspace_cache.allocate(
            nbytes)
        ...
        self.post_loop_store.writeline(
            ...
            f"rsplit_mask, other=triton_helpers.if_mask(rsplit_mask, {constant_repr(default_val)}))"
        )
    ...
    def _lift_helper(self, fn, num_args, dtypes: tuple[torch.dtype,
                                                       ...]) -> str:
        ...
        args = [
            tuple(
                cse.namedvar(f"arg{i}_{n}", dtype=dtypes[n])
                for n in range(num_args)) for i in range(2)
        ]
        signature = ", ".join(
            str(x) for x in itertools.chain.from_iterable(args))
        ...
        class CSEProxy(DefaultHandler):
            ...
            def _default(self, name: str, args: tuple[Any, ...],
    ...
    def scan(
        ...
        if not self.persistent_reduction:
            ...
            result_vars = [
                ...
                ) for full_scan, partial_scan in zip(full_scan_vars,
                                                     partial_scan_vars)
            ]
    ...
    def sort(
        ...
        broadcasted_values = [
            cse_compute(f"tl.broadcast_to({value}, {self.dense_size_str()})",
                        dtype=dtypes[i]) for i, value in enumerate(values)
        ]
        ...
        def cse_multiple(line, n, masks, dtypes):
            ...
            if all(self.cse.contains(cache_key) for cache_key in cache_keys):
                return [self.cse.get(cache_key) for cache_key in cache_keys]
            result_vars = [self.cse.newvar(dtype=dtypes[i])
                           for i in range(n)]  # type: ignore[attr-defined]
            self.compute.writeline(f"{csv(result_vars)} = {line}", )
            for result_var, cache_key in zip(result_vars, cache_keys):
                if masks:
                    result_var.mask_vars = masks  # type: ignore[attr-defined]
        ...
        rnumel = "None" if self._has_constant_mask(
            self.range_trees[-1]) else "rnumel"
    ...
    def codegen_body(self):
        ...
        if self.inside_reduction and len(loop_trees) > 0:
            ...
            for level, tree in enumerate(loop_trees):
                with self.body.indent(offset=level):
                    ...
                    loop_end = ("rsplit_end" if self.cooperative_reduction else
                                f"{prefix}numel")
                    self.body.writeline(
                    ...
                    for block_ptr, advancement in self.pointer_advancements[
                        ...
                        if level < len(loop_trees) - 1:
                            ...
                            prev_advancement = self.pointer_advancements[
                                prev_tree.symt][block_ptr]
                            prev_block = TritonSymbols.get_block_size(
                                prev_tree)
                            prev_num_iter = CeilDiv(prev_tree.numel,
                                                    prev_block)
                            advancement = [
                                cur - prev * prev_num_iter for cur, prev in
                                zip(advancement, prev_advancement)
                            ]
        ...
        if self.cooperative_reduction and (self.post_loop_combine
        ...
        if self.need_numel_args():
            ...
            for arg in numel_args:
                if isinstance(arg, int):
                    ...
                    args.append(str(V.graph.sizevars.size_hint(
                        arg.inner_expr)))
                elif isinstance(arg, sympy.Expr):
                    args.append(str(V.graph.sizevars.size_hint(arg)))
                else:
                    raise ValueError(
                        f"Unsupported numel argument type: {type(arg)}")
        return args
    ...
    def codegen_kernel_benchmark(self, num_gb):
        ...
        with result.indent():
            ...
            with result.indent():
                result.writeline(V.graph.device_ops.set_device(
                    index))  # no-op to ensure context
    ...
    def inductor_meta_common():
        inductor_meta = {
            ...
            config.assert_indirect_indexing,
            "autotune_local_cache":
            config.autotune_local_cache,
            "autotune_pointwise":
            ...
            config.max_autotune_pointwise,
            "min_split_scan_rblock":
            config.triton.min_split_scan_rblock,
            "spill_threshold":
            config.triton.spill_threshold,
            "store_cubin":
            config.triton.store_cubin,
        }
        ...
        if config.profile_bandwidth:
            ...
            inductor_meta[
                "profile_bandwidth_regex"] = config.profile_bandwidth_regex
            inductor_meta[
                "profile_bandwidth_output"] = config.profile_bandwidth_output
            inductor_meta[
    ...
    def codegen_kernel(self, name=None):
        ...
        if self.fixed_config:
            ...
            heuristics_line = f"""
                ...
                @triton.jit
            """
        _ = heuristics_line  # type: ignore[assignment]
        # code.splice(heuristics_line)
        result_params, mojo_args_str = gen_mojo_args(argdefs, signature)
        code.writeline(
            f"fn {name or str(Placeholder.KERNEL_NAME)}{result_params}{mojo_args_str}:")
        with code.indent():
        ...
        binding = make_binding_template(str(Placeholder.KERNEL_NAME), signature)
        result_build = make_build_module(str(Placeholder.KERNEL_NAME), binding, result, argdefs, signature, mojo_args_str, self)
        print(result_build)
        return result_build
    ...
    def _get_persistent_RBLOCK(rnumel):
        ...
        if isinstance(rnumel, (sympy.Integer, int)):
            ...
            while not V.graph.sizevars.statically_known_leq(rnumel, val):
                if val > 16 * 1024:
                    raise ValueError(
                        f"Failed to find static RBLOCK for {rnumel}")
                val *= 2
        return val
        ...
        for tree in self.range_trees:
            if not tree.is_reduction or self.inside_reduction:
                ...
                if is_static_integer(simplified_tree_numel):
                    code.writeline(
                        f"{tree.prefix}numel = {int(simplified_tree_numel)}")
            ...
            if tree.is_reduction and self.persistent_reduction:
                ...
                code.writeline(
                    f"{tree.prefix.upper()}BLOCK: tl.constexpr = {val}")
    ...
    def iteration_ranges_ranges_code(self, entry: IterationRangesRoot) -> str:
        ...
        if (self.cooperative_reduction and self.persistent_reduction
                and entry.is_reduction):
            suffix = f"{suffix} + rsplit_start"
        return f"tl.arange[blocked_1d, 0, {entry.prefix.upper()}BLOCK](){size}{suffix}"

    def iteration_ranges_scalar_code(self, entry: IterationRangesRoot,
                                     value: Any) -> str:
        index_dtype = self.index_dtype
    ...
    def _has_constant_mask(self, tree: IterationRangesRoot) -> bool:
        ...
        if V.graph.sizevars.statically_known_multiple_of(
                tree.numel, max_block):
            return (tree.grid_dim != 1 or tree.has_zdim
                    or V.graph.sizevars.statically_known_leq(
                        tree.numel, get_max_y_grid()))
    ...
    def get_reduction_prefixes(self) -> list[str]:
        return [
            prefix_str[symt] for symt in list(TritonSymbols.reduction_types)
            [:self.num_reduction_dims]
        ]
    ...
    def codegen_reduction_numels(self, buffer: IndentedBuffer) -> None:
        ...
        reduction_trees = [
            tree for tree in self.range_trees if tree.is_reduction
        ]
        rnumel = " * ".join(
        ...
        rn_blocks = [
            TritonSymbols.block_sizes[tree.symt] for tree in self.range_trees
            if tree.is_reduction
        ]
        rblock = sympy_product(rn_blocks)
        buffer.splice(f"RBLOCK: tl.constexpr = {self.kexpr(rblock)}")

    def _get_reduction_symbols(self, suffix: str,
                               **kwargs) -> list[sympy.Symbol]:
        """
        Helper to initialize symbols like rn_numel, rn_base, etc.
        """
        rn_prefixes = self.get_reduction_prefixes()
        return [
            sympy.Symbol(f"{prefix}{suffix}", **kwargs)
            for prefix in rn_prefixes
        ]
    ...
    def _get_reduction_index_coeffs(self) -> list[sympy.Expr]:
        ...
        """
        rn_prefixes = self.get_reduction_prefixes()
        rn_numels = self._get_reduction_symbols("numel",
                                                integer=True,
                                                positive=True)
        return [
            sympy_product(rn_numels[idx + 1:])
            for idx in range(len(rn_prefixes) - 1)
        ] + [sympy.Integer(1)]

    def _flatten_reduction_indices(self,
                                   multi_inds: list[sympy.Expr]) -> sympy.Expr:
        """
    ...
    def codegen_reduction_indices(self, buffer: IndentedBuffer) -> None:
        ...
        rn_offsets = self._get_reduction_symbols("offset",
                                                 integer=True,
                                                 nonnegative=True)
        rn_inds = self._get_reduction_symbols("index",
                                              integer=True,
                                              nonnegative=True)
    ...
    def iteration_ranges_codegen_header(self, entry: IterationRangesRoot,
                                        code: IndentedBuffer) -> None:
        x = entry.prefix
        if entry.is_loop:
...
class TritonScheduling(SIMDScheduling):
    ...
    def codegen_comment(self, node_schedule):
        wrapper = V.graph.wrapper_code
        origins, _detailed_origins = get_kernel_metadata(
            node_schedule, wrapper)
        if origins:
        ...
        if config.debug_fusion:
            ...
            if not any(
                    isinstance(n, ForeachKernelSchedulerNode)
                    for n in node_schedule):
                # We probably should look what are the nodes inside a foreach
                # schedule node
                node_names = [
                    n.get_name() for n in node_schedule
    ...
    def define_kernel(self, src_code, node_schedule, kernel):
        ...
        if src_code in wrapper.src_to_kernel:
            ...
            kernel_name = "_".join([
                "triton", kernel_category, fused_name,
            ...
            src_code = src_code.replace(str(Placeholder.DESCRIPTIVE_NAME),
                                        kernel_name)
            src_code = src_code.replace(str(Placeholder.KERNEL_NAME),
                                        subs_name)
            ...
            _basename, _, kernel_path = get_path(code_hash(src_code.strip()),
                                                 "py")
            ...
            compile_wrapper.writeline(
                f"async_compile.triton({subs_name!r}, '''")

            compile_wrapper.splice(src_code, strip=True)
            current_device = V.graph.get_current_device_or_throw()
            compile_wrapper.writeline(
                f"''', device_str='{current_device.type}')")

            metadata_comment = f"# kernel path: {kernel_path}"
            origins, detailed_origins = get_kernel_metadata(
                node_schedule, wrapper)
            metadata_comment += "\n" + origins + "\n" + detailed_origins
    ...
    def benchmark_fused_nodes(self,
                              nodes,
                              n_spills_threshold=8) -> tuple[float, str]:
        """
        Benchmark fused list of nodes and return the execution time
        in milliseconds on randomly generated inputs.
        """
        src_code = self.generate_kernel_code_from_nodes(nodes,
                                                        benchmark_kernel=True)
        mod = PyCodeCache.load(src_code)
        return self.benchmark_codegened_module(
            mod,
            n_spills_threshold,
            node_names=OrderedSet(n.get_name() for n in nodes))

    def benchmark_codegened_module(
            self,
            mod,
            n_spills_threshold=8,
            node_names: Optional[OrderedSet[str]] = None) -> tuple[float, str]:
        ...
        with (
            ...
            node_names = (node_names if node_names is not None else OrderedSet(
                ["unknown"]))
            log.debug(
    ...
    def create_kernel_choices(  # type: ignore[override]
        ...
        if not TritonKernel.has_persistent_RBLOCK(
        ...
        kernel_kwargs = V.choices.triton_kernel_kwargs(kernel_type,
                                                       kernel_features,
                                                       kernel_args,
                                                       kernel_kwargs)
        kernel = kernel_type(*kernel_args, **kernel_kwargs)
        return self.add_multi_kernel_choices(kernel, kernel_args,
                                             kernel_kwargs)
    ...
    def add_multi_kernel_choices(
        ...
        if optional_cooperative:
            ...
            if V.graph.sizevars.statically_known_leq(rnumel, 65536):
                kernels.append(other := self.kernel_type(
                    *kernel_args,
                    **kernel_kwargs,
...
def debug_triton_code(node: BaseSchedulerNode) -> list[str]:
    ...
    assert multi_template is None or isinstance(multi_template,
